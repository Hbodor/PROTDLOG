...
let zombieGenerator = 0 // to store the interval that generats zombies
...
function StartGame(level) {
    ...
    GameIsPaused = false;
    s.updateMoney(100);
    grid.statBar.healthBar.life=Lives[5];
    grid.statBar.healthBar.bar.style.width = 100 + "%";
    grid.statBar.face.appendChild(grid.statBar.healthBar);
    
    k = new King(2, 0);
    put(k, grid);
    AutoAttack(k, grid, s)
    zombieGenerator = setInterval(function () { if (!pause) { GenerateNewZombie(k, grid) } }, (15 - (level - 1)*5) * 1000);
}
